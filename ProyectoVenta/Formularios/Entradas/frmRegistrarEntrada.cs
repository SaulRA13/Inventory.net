...
namespace ProyectoVenta.Formularios.Entradas
{
    public partial class frmRegistrarEntrada : Form
    {
        ...
        private void frmRegistrarEntrada_Load(object sender, EventArgs e)
        {
            ...
            {
                SQLiteCommand cmd = new SQLiteCommand("SELECT NombreCompleto FROM PROVEEDOR", conexion);
                conexion.Open();
                SQLiteDataReader registro = cmd.ExecuteReader();
                while (registro.Read())
                {
                    txtnomproveedor.Items.Add(registro["NombreCompleto"].ToString());
                }
                conexion.Close();
        ...
        private void btnagregar_Click(object sender, EventArgs e)
        {
            if (txtcodigoproducto.Text.Trim() == "") {
                MessageBox.Show("Debe ingresar el codigo de lente", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }

            if (txtdescripcionproducto.Text.Trim() == "")
            {
                MessageBox.Show("Debe ingresar el codigo de plente", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
        ...
        private void btnguardarentrada_Click(object sender, EventArgs e)
        {
            if (txtnumerodocumento.Text.Trim() == "") {
                MessageBox.Show("Debe ingresar el numero de entrada", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            if (txtdocproveedor.Text.Trim() == "")
            {
                MessageBox.Show("Debe ingresar el numero de tienda", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            if (txtnomproveedor.Text.Trim() == "")
            {
                MessageBox.Show("Debe ingresar el nombre de la tienda", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }

            if (dgvdata.Rows.Count < 1) {
                MessageBox.Show("Debe ingresar algun lente", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
