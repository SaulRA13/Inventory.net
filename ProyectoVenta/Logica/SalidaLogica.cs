...
namespace ProyectoVenta.Logica
{
    public class SalidaLogica
    {
        ...
        public int RegistrarApartado(Salida obj, out string mensaje)
        {
            ...
            using (SQLiteConnection conexion = new SQLiteConnection(Conexion.cadena))
            {
                try
                {
                    ...
                    foreach (Apartado de in obj.olistaApartado)
                    {
                        query.AppendLine(string.Format("insert into APARTADO(IdSalida,IdProducto,CodigoProducto,DescripcionProducto,CategoriaProducto,AlmacenProducto,PrecioVenta,Cantidad,SubTotal) values({0},{1},'{2}','{3}','{4}','{5}','{6}',{7},'{8}');",
                            "(select id from _TEMP)",
                            de.IdProducto,
                            de.CodigoProducto,
                            de.DescripcionProducto,
