        ...
        public List<Inventario> Resumen(string fechainicio = "", string fechafin = "")
        {
            ...
            try
            {
                using (SQLiteConnection conexion = new SQLiteConnection(Conexion.cadena))
                {
                    ...
                    query.AppendLine("select p.IdProducto,sum(de.Cantidad)[Entradas],sum(CAST(de.SubTotal as NUMERIC))[TotalEgresos],e.NombreProveedor[Tienda] from PRODUCTO p");
                    ...
                    query.AppendLine("left join (");
                    query.AppendLine("select p.IdProducto,sum(ds.Cantidad)[Salidas],sum(CAST(ds.SubTotal as NUMERIC))[TotalIngresos],s.NombreCliente[NomCliente] from PRODUCTO p");
